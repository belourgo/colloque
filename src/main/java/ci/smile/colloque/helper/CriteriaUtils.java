...
import ci.smile.colloque.helper.enums.OperatorEnum;
			...
			case "Float":
				switch (operator) {
					...
					case OperatorEnum.BETWEEN:
						params.put(fieldName + "_start", start);
						params.put(fieldName + "_end", end);
						req += String.format(" %1$s >= :%2$s and %1$s <= :%3$s ", jpqlFieldName, fieldName + "_start", fieldName + "_end");
						break;
			...
			case "Date":
				switch (operator) {
					...
					case OperatorEnum.BETWEEN:
						...
						params.put(fieldName + "_start", start);
						params.put(fieldName + "_end", end);
						req += String.format(" DATE(%1$s) >= DATE(:%2$s) and DATE(%1$s) <= DATE(:%3$s) ", jpqlFieldName, fieldName + "_start", fieldName + "_end");
						break;
					case OperatorEnum.BETWEEN_OUT:
						start = (T) new SimpleDateFormat("dd/MM/yyyy").parse(start.toString());
						end = (T) new SimpleDateFormat("dd/MM/yyyy").parse(end.toString());
